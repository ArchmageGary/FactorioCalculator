...
def main_func(name, material, machine_count):
    ...
    # Add a node of the current "name" to list to be turned into diagram
    # all_blocks.append(ELB(f'{material}\nLayer {layer}\n{mach_count:.1f} machines'))
    current_block = ELB(f'{material}\nLayer {layer}\n{mach_count:.1f} machines')
...
with Diagram("Factory", show=True,outformat='jpg'):
    ...
    # Activate block diagram
    l1 = ELB(f'{bottle[0]}\n{bottle[1]:.1f} machines')
    this_block = []
    all_blocks = []
